...
function addPizza(name, size, price) {
    ...
    if (found)
        ...
        cart.push({ id, name, size, unitPrice: price, qty: 1, border: "Sem borda" });
    updateCartButton();
...
function addDrink(name, price) {
    ...
    if (found)
        ...
        cart.push({ id, name, unitPrice: price, qty: 1 });
    updateCartButton();
...
function openCart() {
    ...
    select.addEventListener("change", () => payment = select.value);
    ...
    function renderItems() {
            ...
            cart.forEach((item) => {
                ...
                if (item.size) {
                    ...
                    borderSelect.addEventListener("change", () => {
                        item.border = borderSelect.value;
                        renderItems();
                    });
                ...
                qty.append(button("−", () => { item.qty = Math.max(1, item.qty - 1); updateCartButton(); renderItems(); }, { background: "#f0e5d1", color: "#2c1409", width: "34px", height: "34px", padding: "0" }), Object.assign(document.createElement("strong"), { textContent: String(item.qty) }), button("+", () => { item.qty += 1; updateCartButton(); renderItems(); }, { background: "#f0e5d1", color: "#2c1409", width: "34px", height: "34px", padding: "0" }));
        ...
        const continueBtn = button("Continuar", () => {
            if (!cart.length)
                return toast("Adicione pelo menos um item");
            close();
            openCheckout();
        });
...
function openCheckout() {
    ...
    const send = button("Enviar pedido no WhatsApp", () => {
        ...
        window.open(`https://wa.me/${6192927072}?text=${encodeURIComponent(message)}`, "_blank", "noopener,noreferrer");
    });
